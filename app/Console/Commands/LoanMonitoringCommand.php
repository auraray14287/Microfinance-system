...
class LoanMonitoringCommand extends Command
{
    ...
    private function checkDefaultedLoans(): int
    {
        ...
        // Only mark as defaulted if next_payment_date is 30+ days overdue
        $thresholdDate = now()->subDays(30)->toDateString();
        $defaultedLoans = Loan::whereIn('loan_status', ['approved', 'partially_paid'])
            ->whereNotNull('next_payment_date')
            ->where('next_payment_date', '<', $thresholdDate)
            ->where('balance', '>', 0)
            ->get();
        $count = 0;
